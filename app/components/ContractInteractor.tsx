...
const ContractInteractor = () => {
  ...
  const handleCall = async (fnInfo: FunctionInfo) => {
    if (loadingSignatures[fnInfo.signature]) {
      return;
    }
    ...
    // Keep loading feedback visible even when the wallet responds immediately.
    const loadingFeedback = new Promise<void>((resolve) => window.setTimeout(resolve, 300));
    try {
